...
└──────────────────┘                                         │
     ...
     │  7. Busca usuário no banco local (pré-cadastrado)
     │  8. Gera JWT interno (access_token + refresh_token)
     │  9. Seta cookies HttpOnly e redireciona para dashboard
     ▼
┌──────────┐
...
class RevokedToken(Base):
    """Blacklist de tokens JWT revogados (logout, rotação)."""
...
**Sobre o `hashed_password`**:
Mesmo usando SSO exclusivo, mantemos o campo `hashed_password` como `NOT NULL` com um valor placeholder (hash de string aleatória). Isso simplifica o modelo e permite reativar login local em emergências sem migração.
...
def get_user_by_username_icase(db: Session, username: str) -> Optional[User]:
    """Busca por username (case-insensitive). Fallback quando email não encontra."""
    return db.query(User).filter(
...
# ═══════════════════════════════════════════════════════════════
# PROTEÇÃO 1: SECRET_KEY insegura bloqueia startup em produção
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# JWT: Validação e Revogação (Blacklist)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# Security Logger — formato JSON estruturado para auditoria
# ═══════════════════════════════════════════════════════════════
security_logger = logging.getLogger("security")
...
# ═══════════════════════════════════════════════════════════════
# Middleware 1: Security Headers (CSP, X-Frame-Options, etc.)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# Middleware 2: Autenticação Global + Auto-refresh de Token
# ═══════════════════════════════════════════════════════════════
class GlobalAuthMiddleware(BaseHTTPMiddleware):
    ...
    2. Tem access_token válido? → passa direto
    3. Access_token inválido mas tem refresh_token válido? → gera novo access_token,
       seta como cookie, e passa direto (usuário nem percebe)
    4. Nenhum token válido? → redireciona para /login (páginas) ou 401 (API)
    """
    async def dispatch(self, request: Request, call_next):
        ...
        if not payload:
            # Access token inválido/expirado — tentar refresh
            ...
            if refresh_token:
                ...
                if refresh_payload:
                    try:
                        ...
                        # Revogar access token antigo (best-effort)
                        try:
...
# ═══════════════════════════════════════════════════════════════
# Setup: Registra todos os middlewares e exception handlers
# ═══════════════════════════════════════════════════════════════
def setup_security(app: FastAPI):
    ...
    # ORDEM IMPORTA: SecurityHeaders antes de GlobalAuth
...
# Estado pendente para proteção CSRF do OAuth
...
# ═══════════════════════════════════════════════════════════════
# Endpoint: Verificar se SSO está configurado
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# Endpoint: Callback SSO (Microsoft redireciona para cá)
# ═══════════════════════════════════════════════════════════════
...
async def microsoft_callback(
    ...
    # 3. Código de autorização presente?
    if not code:
        return RedirectResponse(
    ...
    try:
        # 4. Trocar authorization_code por tokens
        ...
        # 5. Extrair identidade do id_token_claims
        ...
        # 6. Buscar usuário no banco local (case-insensitive)
        user = crud.get_user_by_email_icase(db, email)

        if not user:
            # Fallback: buscar por username (para quando email != username)
        ...
        # 7. Verificar role (autorização)
        allowed_roles = ["admin", "user"]  # ajuste conforme sua aplicação
        ...
        # 9. Setar cookies e redirecionar
        redirect = RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)

        # Limpar cookies legados (de paths antigos)
        ...
        redirect.set_cookie(
            ...
            samesite="lax",
            max_age=86400,       # 24h (cookie vive mais que o token — refresh cuida)
            path="/"
        )
        redirect.set_cookie(
            key="refresh_token",
            ...
            samesite="lax",
            path="/api/auth",    # Restrito ao path de auth (mínimo privilégio)
            secure=IS_PRODUCTION
        )
        return redirect
...
### 6.8 Registro de Middlewares no `main.py`
...
# ═══════════════════════════════════════════════════════════════
# Rota de Health Check (deve estar ANTES do middleware de auth)
# ═══════════════════════════════════════════════════════════════
@app.get("/health")
async def health():
    return {"status": "ok"}
...
# ═══════════════════════════════════════════════════════════════
# Registrar segurança e routers no lifespan
# ═══════════════════════════════════════════════════════════════
...
# ... re-registrar rotas estáticas e de saúde aqui ...
...
1. Rota `/health` deve ser registrada **antes** do lifespan (para o health check funcionar durante startup)
2. `setup_security(app)` registra middlewares — a ordem interna importa: `SecurityHeaders` antes de `GlobalAuth`
3. Routers são registrados **depois** dos middlewares
...
| NÃO FAÇA | POR QUÊ | FAÇA ISSO |
|---|---|---|
| Auto-criar usuários no callback SSO | Qualquer pessoa com conta Microsoft poderia acessar sua aplicação | Exija pré-cadastro por admin. SSO apenas identifica, não autoriza |
...
```python
# Registrar saúde e login ANTES de tudo (instantâneo)
@app.get("/health")
async def health():
    return {"status": "ok"}

# Carregar routers pesados em background DEPOIS do uvicorn estar respondendo
async def load_heavy_routers():
    import asyncio
    await asyncio.sleep(1)  # Dar tempo para health check passar
...
### Banco de Dados
- [ ] Tabela `users` existe com pelo menos um admin pré-cadastrado
...
- [ ] Usuário não cadastrado mostra mensagem clara
