...
def rerank(
    ...
    sys_prompt = (
        ...
        "contém 'table', 'tabela', 'financial_table' ou 'portfolio_row' (linha "
        "individual de carteira) ou cujo snippet exibe a métrica numérica "
        "explicitamente DEVEM vir no topo.\n"
        ...
        "'agora').\n"
        "5) RAG V3.6 — Se a query menciona 'carteira', 'composição', 'alocação', "
        "'pesos', 'todos os ativos/fundos/papéis', 'liste', ou pede pelo peso/"
        "participação de um ticker específico, priorize FORTEMENTE candidatos com "
        "block_type 'tabela', 'financial_table' ou 'portfolio_row' do mesmo "
        "material. Blocos de texto narrativo do mesmo material vêm DEPOIS dos blocos "
        "tabulares. Quando vários portfolio_row do mesmo material existirem, mantenha-"
        "os agrupados na ordem original (linha 1, linha 2, ...) — não os intercale "
        "com blocos de outros materiais.\n"
        "6) Em empate, mantenha a ordem original."
    )
    user_payload = {
        "query": query,
        "candidates": items,
